  ...
  try {
    ...
    for (let i = 0; i < jobIds.length; i++) {
      ...
      try {
        ...
        if (!jobResponse.data || !jobResponse.data.job || jobResponse.data.job.status !== 'completed') {
        ...
        if (stemType === 'other') {
          ...
          const otherStem = jobResponse.data.job.stemAssets?.find(asset => asset.name === 'other.wav');
          stemUrl = otherStem?.link;
        } else {
          // For other stems, look in stemAssets array with their respective names
          const stem = jobResponse.data.job.stemAssets?.find(asset => asset.name === `${stemType}.wav`);
          if (stem) {
            stemUrl = stem.link;
          } else {
        ...
        if (!stemUrl) {
          console.error(`No URL found for ${stemType} stem in job response. Job data:`, JSON.stringify(jobResponse.data.job, null, 2));
          continue;
        }
        ...
        const stemPath = path.join(songStemsDir, `${stemType}_stem.wav`);
